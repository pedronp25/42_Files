...
/*   Updated: 2025/06/19 00:22:15 by pedromig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child1(t_pipex *pp, char *cmd, char **envp)
{
	...
	close(pp->pipe_fd[1]);
	close(pp->pipe_fd[0]);
	pp->cmd1_args = ft_split_pipex(cmd, ' ');
	pp->cmd1_path = pipex_find_path(pp, envp, pp->cmd1_args[0]);
	ft_printf("Running: %s\n", pp->cmd1_path);
	if (execve(pp->cmd1_path, pp->cmd1_args, envp) == -1)
...
void	child2(t_pipex *pp, char *cmd, char **envp)
{
	...
	close(pp->fd2);
	close(pp->pipe_fd[1]);
	pp->cmd2_args = ft_split_pipex(cmd, ' ');
	pp->cmd2_path = pipex_find_path(pp, envp, pp->cmd2_args[0]);
	ft_printf("Running: %s\n", pp->cmd2_path);
	if (execve(pp->cmd2_path, pp->cmd2_args, envp) == -1)
